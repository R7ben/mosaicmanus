...
export default function QuizHubPage() {
  ...
  return <main className="quiz-hub-page"><header className="workspace-header"><a href={teacher ? "/teacher" : "/student"} className="brand"><span className="mosaic-mark">M</span><span>Mosaic<span>Classroom</span></span></a><a className="text-button" href={teacher ? "/teacher" : "/student"}><ArrowLeft size={15} />Back to dashboard</a></header><section className="quiz-hub-content"><div className="quiz-hub-heading"><div><div className="eyebrow"><BookOpenCheck size={14} />{teacher ? "Teacher tools" : "Student learning"}</div><h1>{teacher ? "Quizzes" : "My quizzes"}</h1><p>{teacher ? "Create, manage, and review classroom assessments from one visible place." : "Find available practice and review your completed quiz history."}</p></div>{teacher && <a className="btn btn--student" href="/teacher/quiz/create"><Plus size={16} />Create quiz</a>}{!teacher && <a className="btn btn--student" href="/student/review"><Sparkles size={16} />Review results</a>}</div><div className="quiz-toolbar"><label><Search size={15} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search quizzes" /></label><select value={status} onChange={(event) => setStatus(event.target.value)}><option>All</option><option>Upcoming</option><option>Active</option><option>Completed</option><option>Draft</option></select><select value={sort} onChange={(event) => setSort(event.target.value)}><option value="newest">Newest</option><option value="oldest">Oldest</option></select></div><div className="quiz-card-grid">{filtered.length ? filtered.map((quiz) => <article className="quiz-hub-card" key={quiz.id}><div className="quiz-hub-card__top"><span className="status-pill">{"status" in quiz ? quiz.status : quiz.published ? "Active" : "Draft"}</span><span>{quiz.isTeacherQuestion ? "Teacher question" : "Classroom quiz"}</span></div><h2>{quiz.title}</h2><p>{quiz.questionCount} question{quiz.questionCount === 1 ? "" : "s"}{"score" in quiz && ` · Score ${quiz.score}`}</p>{teacher ? <div className="quiz-hub-card__actions"><a className="text-button" href="/teacher/quiz/create">Edit / add questions <ArrowRight size={14} /></a></div> : <a className="btn btn--soft" href="/kiosk">Start practice <ArrowRight size={15} /></a>}</article>) : <div className="quiz-hub-empty"><h2>No quizzes match this view.</h2><p>{teacher ? "Create your first quiz to make an assessment visible to students." : "Your teacher has not published a quiz here yet."}</p>{teacher && <a className="btn btn--student" href="/teacher/quiz/create">Create a quiz <Plus size={15} /></a>}</div>}</div></section></main>;
}
