...
import { bumpSessionVersion } from "./db";
...
export const appRouter = router({
  ...
  auth: router({
    ...
    logout: publicProcedure.mutation(async ({ ctx }) => { if (ctx.user) await bumpSessionVersion(ctx.user.openId); const cookieOptions = getSessionCookieOptions(ctx.req); ctx.res.clearCookie(COOKIE_NAME, { ...cookieOptions, maxAge: -1 }); return { success: true } as const; }),
  }),
  mosaic: router({
    ...
    syncOffline: publicProcedure.input(z.object({ answers: z.array(z.object({ learnerId: z.string(), option: z.string(), confidence: z.enum(["guessed", "unsure", "knew"]), questionId: z.string().optional() })) })).mutation(async ({ input }) => { let synced = 0; for (const answer of input.answers) { const question = PULSE_QUESTIONS.find((item) => item.id === answer.questionId) ?? PULSE_QUESTIONS[0]; const correct = answer.option === question.answer; const classification = classifyAnswer(answer.option, correct); const persisted = await persistAnswer({ learnerId: answer.learnerId, option: answer.option, correct, confidence: answer.confidence, feedback: deterministicFeedback(correct), questionId: question.id, ...classification }); if (persisted) synced += 1; } return { synced }; }),
    answerQuiz: publicProcedure.input(z.object({ learnerId: z.string(), option: z.string(), confidence: z.enum(["guessed", "unsure", "knew"]), questionId: z.string().optional() })).mutation(async ({ input }) => { const question = PULSE_QUESTIONS.find((item) => item.id === input.questionId) ?? PULSE_QUESTIONS[0]; const correct = input.option === question.answer; const state = await readClassroomState(); const feedback = await Promise.race([generateAdaptiveFeedback({ subject: state.classroom.subject, topic: "Forces & Motion", option: input.option, correct, confidence: input.confidence }), new Promise<string>((resolve) => setTimeout(() => resolve(deterministicFeedback(correct)), 7000))]); const classification = classifyAnswer(input.option, correct); const persisted = await persistAnswer({ learnerId: input.learnerId, option: input.option, correct, confidence: input.confidence, feedback, questionId: question.id, ...classification }); if (persisted) fallbackLearners = fallbackLearners.map((learner) => learner.id === input.learnerId ? persisted : learner); else fallbackLearners = fallbackLearners.map((learner) => learner.id === input.learnerId ? { ...learner, tier: correct ? learner.tier : "red", mastery: correct ? Math.min(100, learner.mastery + 6) : Math.max(28, learner.mastery - 13), misconception: correct ? learner.misconception : (question.misconception ?? "Mass and weight are the same thing"), flagged: !correct, recent: "Just now" } : learner); return { correct, learner: persisted ?? fallbackLearners.find((learner) => learner.id === input.learnerId), feedback, reasoning: classification.reasoning, classifierConfidence: classification.classifierConfidence }; }),
